...
class ItemStack:
  ...
  def __init__(self, item: Optional[Item] = None, count: int = 0):
    self.item = item      # 物品類型
    self.count = count    # 數量
    self.durability = None  # 當前耐久度（如果物品有耐久度）
  ...
  def copy(self) -> 'ItemStack':
    """複製物品堆疊"""
    new_stack = ItemStack(self.item, self.count)
    new_stack.durability = self.durability
    return new_stack
  ...
  def split(self, amount: int) -> 'ItemStack':
    ...
    if self.count <= 0:
      ...
      self.clear()
      new_stack = ItemStack(item, actual_amount)
      new_stack.durability = durability
      return new_stack

    new_stack = ItemStack(self.item, actual_amount)
    new_stack.durability = self.durability
    return new_stack
